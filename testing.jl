...
X_3way = cp_als_3way(full_3way, 1, seed=18, init=[A, B, C])
X_dway, _ = cp_als_dway(full_dway, 1, seed=18, init=[A, B, C])
println("Original 3-way tensor: ")
println(construct_kruskal(test_dway))
println("cp als 3way:")
println(construct_kruskal(X_3way))
println("cp als dway:")
println(construct_kruskal(X_dway))
